...
method new() class TDbEmpresas
    ...
    sql:add("emp_crt AS CRT, ")
    sql:add("emp_lucro AS lucro, ")
    // sql:add("emp_PIS AS PIS, ") * Não usado, o cálculo foi feito pela triger na tabela cte (before insert)
    // sql:add("emp_COFINS AS COFINS, ") * Não usado, o cálculo foi feito pela triger na tabela cte (before insert)
